...
import numpy as np
from feature_engineering import generate_lbp_dataframe
...
def create_tier2_ft_df():
    ...
    lbp_df = generate_lbp_dataframe(metadata_df)
...
def scale_data(X_train, X_test, ignore_keywords=None, return_numpy=False):
    ...
    X_test_scaled_df = pd.DataFrame(
        X_test_scaled_part, columns=cols_to_scale, index=X_train.index
    )
